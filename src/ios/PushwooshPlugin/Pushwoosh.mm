...
void pushwoosh_swizzle(Class cls, SEL fromChange, SEL toChange, IMP impl, const char * signature) {
    ...
    if (method) {
        ...
        class_addMethod(cls, toChange, impl, signature);
        ...
        class_addMethod(cls, fromChange, impl, signature);
    }
}
...
// No custom name: React Native looks a TurboModule up by NSClassFromString(name) before it
// consults the RCT_EXPORT_MODULE registry, and "Pushwoosh" is the native SDK's own class.
RCT_EXPORT_MODULE();

#ifdef RCT_NEW_ARCH_ENABLED
- (std::shared_ptr<facebook::react::TurboModule>)getTurboModule:(const facebook::react::ObjCTurboModule::InitParams &)params {
    return std::make_shared<facebook::react::NativePushwooshSpecJSI>(params);
}
#endif
...
RCT_EXPORT_METHOD(init:(NSDictionary*)config success:(RCTResponseSenderBlock)success error:(RCTResponseSenderBlock)error) {
    ...
    // Once per launch push. Neither source clears itself here - gStartPushData is consumed by the
    // legacy onPushOpen:, launchNotification belongs to the SDK - so a second init() (a consent
    // flow initialising after the user agreed, an app switching application code, a Fast Refresh
    // reload) used to replay the same push as pushReceived and pushOpened again and to re-arm a
    // deep link that Linking.getInitialURL() had already consumed, sending the user back to the
    // screen the push opened the first time.
    if (!gStartPushReplayed) {
        gStartPushReplayed = YES;
        ...
        if (gStartPushData) {
            ...
            if (deepLink) {
                ...
                // Writing the link into launchOptions is what makes RCTLinking report it, but only
                // a real bridge accepts the write. Bridgeless reads it from the swizzled
                // getInitialURL.
...
RCT_EXPORT_METHOD(registerForPushNotifications:(RCTResponseSenderBlock)success error:(RCTResponseSenderBlock)error) {
...
RCT_EXPORT_METHOD(postEvent:(NSString*)event attributes:(NSDictionary*)attributes) {
...
RCT_EXPORT_METHOD(addToApplicationIconBadgeNumber:(double)badgeNumber) {
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIApplication sharedApplication].applicationIconBadgeNumber += (NSInteger)badgeNumber;
    });
}
...
RCT_EXPORT_METHOD(loadMessages:(RCTResponseSenderBlock)success fail:(RCTResponseSenderBlock)fail) {
    [PWInbox loadMessagesWithCompletion:^(NSArray<NSObject<PWInboxMessageProtocol> *> *messages, NSError *error) {
        if (error) {
            if (fail) {
                fail(@[ objectOrNull([error localizedDescription]) ]);
            }
            return;
        }
        ...
        if (success) {
            ...
            for (NSObject<PWInboxMessageProtocol> *message in messages) {
                [array addObject:[self inboxMessageToDictionary:message]];
            }
            success(@[ array ]);
        }
    }];
...
API_AVAILABLE(ios(10.0)) {
    ...
    // 0 when this notification is not ours to present: someone else's, a repeat of the same
    // message, or a Pushwoosh push the app asked us not to show an alert for.
    ...
    if (isPushwooshMessage && isNeedToImplementInPlugin && ![lastPushwooshHash isEqualToString:currentHash]) {
        ...
        if ([PushNotificationManager pushManager].showPushnotificationAlert) {
            options = UNNotificationPresentationOptionSound | UNNotificationPresentationOptionBadge | UNNotificationPresentationOptionAlert;
        } else {
    ...
    // UNUserNotificationCenter waits for exactly one answer: a second call is an error it logs,
    // and no call at all leaves the notification unpresented. Both used to be reachable here -
    // our own alert answered and the app's delegate answered again with the same handler, while a
    // notification that is neither ours nor claimed by an app delegate was never answered at all.
    // So whoever answers first decides and the rest is swallowed.
    __block BOOL answered = NO;
    void (^answerOnce)(UNNotificationPresentationOptions) = ^(UNNotificationPresentationOptions presentationOptions) {
        if (answered) {
            return;
        }
        answered = YES;
    ...
    // The SDK's alert goes up straight away, as before.
    if (options != 0) {
        answerOnce(options);
    }

    // The app's own delegate still gets the callback either way. A Pushwoosh message is handed
    // over with a delay, so the SDK presents its own alert first.
        ...
        dispatch_block_t presentationBlock = ^{
            [self->_originalNotificationCenterDelegate userNotificationCenter:center
                                                willPresentNotification:notification
                                                  withCompletionHandler:answerOnce];
        };
...
#pragma mark - Android-only settings

// Declared so the TurboModule spec is fully implemented on iOS: a call from JS is a no-op here
// instead of a TypeError. The settings exist only in the Android SDK.

RCT_EXPORT_METHOD(setMultiNotificationMode:(BOOL)on) {}

RCT_EXPORT_METHOD(setLightScreenOnNotification:(BOOL)on) {}

RCT_EXPORT_METHOD(setEnableLED:(BOOL)on) {}

RCT_EXPORT_METHOD(setColorLED:(double)color) {}

RCT_EXPORT_METHOD(setSoundType:(double)type) {}

RCT_EXPORT_METHOD(setVibrateType:(double)type) {}

RCT_EXPORT_METHOD(setNotificationIconBackgroundColor:(NSString *)color) {}
...
RCT_EXPORT_METHOD(setRichMediaType:(double)type) {
    PWRichMediaPresentationStyle style = (NSInteger)type == 0
